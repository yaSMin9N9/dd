...
import ProfileOnePage from "pages/ProfileOne";
...
const ChoosefromherePage = () => {
  ...
  const handleToggle = async () => {
    ...
    const fetchBooking = async () => {
      setIsloading(true)
      try {
        const response = await axios.get(`http://localhost:8000/user/booking/${param._id}`, {
          headers: {
...
const buttonToggle =() =>{
  ...
  const handleroute = (_id) => {
    navigate(`/choosePayment/${_id}`);
  };
 const handleDes =(_id) =>{
  console.log(_id);
  navigate(`/QuestionDesign/${_id}`)
 }
 const closebuttonn =async () =>{
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8000/user/services/getAdvisors/${param._id}`, {
  ...
  const hi = book?.map(boo => boo.serviceId)
  const hii = book?.map(boo => boo.status)
  const conver = book?.map(boo => boo.conversationId)
console.log(hii.toString() === "pending");
  const handleNo =() =>{
    navigate("/QuestionDesign")
   }
   
 console.log(param._id);
   useFetchServices(setAdvisors,param._id, token);
    ...
    {booking?.bookingStatus === "design" && (
              <>
                <div className="bg-gradient font-dmserifdisplay md:h-[271px] h-[272px] md:px-5 relative w-full">
                  <div className=" bg-cover bg-no-repeat flex flex-col  items-end justify-center m-auto p-[85px] md:px-10 sm:px-5 w-full" style={{ backgroundImage: "url('images/img_div.png')" }}></div>
                  <Text className="mb-8 absolute top-20 right-20 sm:text-[40px] md:text-[46px] text-[50px] text-white-A700" size="txtDMSerifDisplayRegular50">يمكنك محادثة المصمم</Text>
                  <Button style={{cursor:"pointer",marginTop:"30px",color:"#235f80"}} className="bg-white hover:bg-gray-100  font-semibold py-2 px-4 border border-gray-400 rounded shadow" onClick={closebuttonn}>انهاء مرحلةالتصميم</Button>
      
      <PopChat conversationId={booking?.conversationId} style={{position:"absolute", top:"100px",left:"100px"}}/></div>
     </>
)}
    {booking?.bookingStatus === "arbitrationPending"|| booking?.bookingStatus === "pending" &&( <div style={{backgroundColor:"#B4B4B3"}} className="font-dmserifdisplay md:h-[371px] sm:h-[371px] h-[371px] md:px-5 relative w-full">
    ...
    {booking?.bookingStatus === "endArbitration" && (
        <div style={{backgroundColor:"#B4B4B3", width:"100vw"}} className=" md:h-[571px] sm:h-[371px] h-[450px] md:px-5 relative ">
           ...
           <Button style={{cursor:"pointer",margin:"10px"}} className="text-blue_gray-700_01 sm:text-[20px] h-[46px] justify-center mt-[10px] pb-[3px] pt-2 sm:px-5 px-[20px] rounded-[30px] text-xl md:text-[18px] bg-white-A700  " onClick={() => downloadFile(booking?.finalPdfFile)}>تحميل</Button>
            <Button style={{cursor:"pointer",margin:"10px"}} className="text-blue_gray-700_01 sm:text-[20px] h-[46px] justify-center mt-[10px] pb-[3px] pt-2 sm:px-5 px-[20px] rounded-[30px] text-xl md:text-[18px] bg-white-A700  " onClick={handleDes(param._id)}>الانتقال</Button>
      ...
      <div style={{backgroundColor:"#B4B4B3",width:"100vw"}} className="font-dmserifdisplay md:h-[571px] sm:h-[371px] h-[450px] md:px-5 relative w-full">
        <div style={{display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center"}}>
            ...
            <Link className='text-blue_gray-700_01' to={`/ChoosefromArbitrators/${param._id}`} style={{color:"white", margin:"10px",padding:"10px 30px",color:"#000",backgroundColor:"#fff",borderRadius:"30px"}}>نعم</Link>
            ...
            {advisors.map((adv, index) => (
              <div key={index} className="bg-white-A700 m-5 flex flex-col md:gap-10 gap-16 h-auto md:h-auto items-center justify-start pb-[50px] pt-16 md:px-10 sm:px-5 rounded-[32px] shadow-bs1 w-[350px]">
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "250px" }}>
                  <Link to={`/profile/${adv._id}/${param._id}`}>
...
{/* )} */}
                  ...
                  {openAdvisorId === adv._id && (
                    <div style={{ position: "absolute",backgroundColor:"#fff" }} className="mt-2 w-48 bg-white border border-gray-300 rounded shadow-lg">
                      <ul>
                        <Link to={`/paymentBank/${adv._id}/${param._id}`}>
